...
When a weight probe first reaches target, STEP generation stops while the motor remains energized. Eight fresh corrected readings must settle within a 2 g range. A settled result below target explicitly restarts the hardware alarm, ramps back toward cruise, and delays another probe for five hub turns; a result at or above target completes the run. Three consecutive insufficient, excessive-range, or near-target probe revolutions force a stationary settled check instead of allowing unlimited slow probe loops. A 15-second timeout finishes safely rather than holding the machine indefinitely. The completion screen latches final turns and the settled measured weight until the encoder is clicked. Telemetry emits `STEP_RESTART_ARMED` and `STEP_RESTART_FIRST_STEP_delay_us_...` markers so commanded restart time can be compared with the first generated STEP.
...
Before every Fuhgeddabouditinator run, the energized hub turns through one complete slow revolution while empty and captures a 32-bin raw baseline indexed by hub angle. It then stops with the motor still energized and displays `LOAD YARN`; attach the yarn and click before winding begins, or press STOP to cancel. During a preset run, a possible crossing within the larger of 3 g or 15% of the target starts a reversible weighing probe: speed ramps down at 1 motor RPS per second toward 0.5 RPS, then the controller measures a complete hub revolution and subtracts the matching empty angle bin from every sample. If the corrected revolution average is clearly below the approach band, it ramps back to 3.0 RPS and suppresses another probe for five hub turns. A corrected average at or above target enters the settled final-weight check; a result inside the approach band repeats another slow revolution. The winding screen retains the nominal 20 g, 50 g, or 100 g label rather than exposing the configured overage target.
...
6. For weight-control trials, issue `WEIGHT TARGET 20` (or the desired measured target), choose a deliberately generous turn ceiling, and keep STOP accessible. Confirm `WEIGHT_APPROACH` and `WEIGHT_TARGET_REACHED` events against the raw and filtered weight columns. Use `WEIGHT OFF` after the trial.
