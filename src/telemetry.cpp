...
void Telemetry::start()
{
 ...
 {
  static const char* programs[]={"Mini 20g","Half 50g","Full 100g"};
  if(weightCapabilityEnabled)DiagnosticsTransport::beginTelemetryStream("Fuhgeddabouditinator",autoCountMode?"Count":programs[uint8_t(activeFuhProgram)],uint16_t(selectedTurns));
  else if(autoCountMode)DiagnosticsTransport::beginTelemetryStream("Turninator","Count",uint16_t(selectedTurns));
...
void Telemetry::executeCommand(const char* command,Print& output,bool logAccessAllowed){
 ...
 if(c=="LOAD CELLS"){
  ...
  output.print("weight_g,");output.print(LoadCells::weightGrams(loadCells),2);output.print(",tare_status,");output.print(LoadCells::tareStatusName(tare.status));output.print(",profile_tare,");output.println(LoadCells::profileTareValid()?1:0);return;
 }
